...
const images = [
  ...
  "./assets/stamp44.png",
  "./assets/stamp5.png"
]; // <-- hier kannst du beliebige Bilder hinzufügen

document.body.addEventListener("click", (e) => {
  const stamp = document.createElement("img");
  stamp.src = images[Math.floor(Math.random() * images.length)];
  stamp.classList.add("stamp");

  // Zufällige Rotation und Skalierung
  const rotation = Math.random() * 60 - 30; // -30° bis +30°
  const scale = 0.8 + Math.random() * 0.4;  // 0.8x bis 1.2x

  stamp.style.left = e.pageX + "px";
  stamp.style.top = e.pageY + "px";
  stamp.style.transform = `translate(-50%, -50%) rotate(${rotation}deg) scale(0)`;
  ...
  // Entferne Stempel nach 5 Sekunden (optional)
  setTimeout(() => {
    stamp.remove();
  }, 5000);
});
